...
function FloatingPencils() {
  const [pencil1Color, setPencil1Color] = React.useState('#3de8ff');
  const [pencil2Color, setPencil2Color] = React.useState('#7c4dff');
  const pencil1Ref = React.useRef<HTMLDivElement>(null);
  const pencil2Ref = React.useRef<HTMLDivElement>(null);
  const pencil1WasVisible = React.useRef(true);
  const pencil2WasVisible = React.useRef(true);
  ...
  React.useEffect(() => {
    const checkVisibility = () => {
      // Check pencil 1
      if (pencil1Ref.current) {
        const rect = pencil1Ref.current.getBoundingClientRect();
        const isVisible = 
        ...
        // If pencil was off-screen and is now visible, change color
        if (!pencil1WasVisible.current && isVisible) {
          setPencil1Color((prev) => getRandomColor(prev));
        }
        pencil1WasVisible.current = isVisible;
      }

      // Check pencil 2
      if (pencil2Ref.current) {
        const rect = pencil2Ref.current.getBoundingClientRect();
        const isVisible = 
          rect.top < window.innerHeight &&
          rect.bottom > 0 &&
          rect.left < window.innerWidth &&
        ...
        // If pencil was off-screen and is now visible, change color
        if (!pencil2WasVisible.current && isVisible) {
          setPencil2Color((prev) => getRandomColor(prev));
        }
        pencil2WasVisible.current = isVisible;
      }
    };
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      <div
        ref={pencil1Ref}
        className="absolute top-[25%] w-24 h-6 animate-drift-right transition-colors duration-1000"
        style={{ opacity: 0.2 }}
      >
        <ColoringPencil color={pencil1Color} />
      </div>
      <div
        ref={pencil2Ref}
        className="absolute top-[65%] w-20 h-5 animate-drift-left-slow transition-colors duration-1000"
        style={{ opacity: 0.15, animationDelay: '3s' }}
      >
        <ColoringPencil color={pencil2Color} />
      </div>
    </div>
  );
}
